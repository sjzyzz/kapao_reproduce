...
if __name__ == "__main__":
    ...
    parser.add_argument('--img-path',
                        default='res/crowdpose_100024.jpg',
                        help='path to image')
    ...
    parser.add_argument('--no-kp-dets',
                        action='store_true',
                        help='do not use keypoint object to do the fusion')
    ...
    model = Model(args.cfg,
                  ...
                  nc=data['nc'],
                  anchors=None,
                  num_coords=data['num_coords']).to(device)
    ...
    if args.bbox:
        ...
        bboxes = scale_coords(img.shape[2:], bbox_dets[0][:, :4],
                              img0.shape[:2]).round().detach().cpu().numpy()
        for x1, y1, x2, y2 in bboxes:
            cv2.rectangle(img0, (int(x1), int(y1)), (int(x2), int(y2)),
                          args.color_pose,
                          thickness=args.line_thick)

    # i think this is the fusion of pose object and kp object
    # but the interface is really ugly lol
    _, poses, _, _, _ = post_process_batch(data, img, [], [[img0.shape[:2]]],
                                           [x.detach() for x in bbox_dets],
                                           [x.detach() for x in kp_dets])
    if args.pose:
        for pose in poses:
